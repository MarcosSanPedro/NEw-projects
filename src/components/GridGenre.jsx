            ...
            <div className="w-screen grid grid-cols-2 md:grid-cols-3  lg:grid-cols-4 gap-4">
                {items.map((item) => (
                    <div key={item.id} className=" w-64 m-auto">
                       <Link to={`/details/${item.title ? "movie" : "tv"}/${item.id}`} key={`${item.title || item.name}-${item.id + Math.random()}`}>
                        <img
                            src={item.poster_path ? `https://image.tmdb.org/t/p/w500${item.poster_path}` : "https://images.placeholders.dev/?width=256&height=256&text=404"}
                            alt={item.title}
